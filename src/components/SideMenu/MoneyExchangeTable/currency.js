...
export const updateCurrency =
  ({ data: currency, setError, availableCurrency, updateCurrencyStates }) => {
  const receivedCurrency = currency.filter(currency => availableCurrency.includes(currency.ccy));
  const newCurrency = [];

  receivedCurrency.map(elem => {
    elem.buy = moneyDisplay(elem.buy) || setError("Unexpected value received");
    elem.sale = moneyDisplay(elem.sale) || setError("Unexpected value received");
